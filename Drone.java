...
public class Drone {
	...
	void move(double simTime) {//移動メソッド
		...
		case GATHERING:
			...
			case gBACK:
				...
				if(gInitX > meetingPlaceX + 100 && gInitY > meetingPlaceY) {
					x += gBackDistance * Math.cos(gBackTheta);
					y += gBackDistance * Math.sin(gBackTheta);
				}
				else if(gInitX <= meetingPlaceX + 100 && gInitY > meetingPlaceY) {
					x -= gBackDistance * Math.cos(gBackTheta);
					y += gBackDistance * Math.sin(gBackTheta);
				}
				else if(gInitX > meetingPlaceX + 100 && gInitY <= meetingPlaceY) {
				...
				else{//(gInitX <= meetingPlaceX + 100 && gInitY <= meetingPlaceY)
					x -= gBackDistance * Math.cos(gBackTheta);
					y -= gBackDistance * Math.sin(gBackTheta);
				}

				endGatheringMove = Math.sqrt(Math.pow(meetingPlaceX + 100 - gInitX, 2) + Math.pow(meetingPlaceY - gInitY, 2));
	...
	double get_gBackTheta() {//召集から帰ってくる場合の角度
		double gBackTheta;
		if(gInitX > (meetingPlaceX + 100) && gInitY > meetingPlaceY) {
			gBackTheta = Math.atan2( gInitY - meetingPlaceY, gInitX - (meetingPlaceX + 100));//角度
		}
		else if(gInitX <= (meetingPlaceX + 100) && gInitY > meetingPlaceY) {
			gBackTheta = Math.atan2( gInitY - meetingPlaceY, (meetingPlaceX + 100) - gInitX);//角度
		}
		else if(gInitX > (meetingPlaceX + 100) && gInitY <= meetingPlaceY) {
			gBackTheta = Math.atan2( meetingPlaceY - gInitY, gInitX - (meetingPlaceX + 120));//角度
		}
		else{//(gInitX <= meetingPlaceX + 100 && gInitY <= meetingPlaceY)
			gBackTheta = Math.atan2( meetingPlaceY - gInitY, (meetingPlaceX + 100) - gInitX);//角度
		}
		return gBackTheta;
	}
}
